...
class Ori_datas(object):
    ...
    def show_figure(self, adaboost):
        ...
        # 绘制各个弱分类器
        for best_stump in adaboost.best_stump_arr:
            if best_stump['dim'] == 1:
                plt.axhline(y=best_stump['thresh_val'], c="blue")  # 添加水平直线
            else:
                plt.axvline(x=best_stump['thresh_val'], c="red")  # 添加垂直直线
        ...
        for x_i in x:
            for y_j in y:
                pred_result = adaboost.ada_classify([x_i, y_j])
                if pred_result == 1:
